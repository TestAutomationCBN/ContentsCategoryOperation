...
Public Class UtillFunc
    ...
    ' Ecxelワークブックから、コンテンツカテゴリクラスのリストを生成する関数
    Public Function ExceltoContentsCategoryList(wb As XLWorkbook) As ArrayList
        ...
        Using wb
            Dim ws = wb.Worksheet("コンテンツカテゴリ設計")
            ...
            While ws.Cell(cnt, 5).Value <> "" Or ws.Cell(cnt, 6).Value <> "" Or ws.Cell(cnt, 7).Value <> "" Or ws.Cell(cnt, 8).Value <> "" Or ws.Cell(cnt, 9).Value <> "" Or ws.Cell(cnt, 10).Value <> "" Or ws.Cell(cnt, 11).Value <> "" Or ws.Cell(cnt, 12).Value <> "" Or ws.Cell(cnt, 13).Value <> ""
                ...
                Dim cat As New ContentsCategory
        ...
        ExceltoContentsCategoryList = catList
    ...
    ' コンテンツカテゴリリストから、テスト仕様書を生成する関数
    Public Sub ContentsCategoryListtoTest(project As String, catList As ArrayList, export_filepath As String)

        '雛形ファイルを開く
        Dim base As New ClosedXML.Excel.XLWorkbook(My.Application.Info.DirectoryPath & "\TestBase\con_base.xlsx")

        'シートを取得
        Dim sheet As ClosedXML.Excel.IXLWorksheet = base.Worksheet("コンテンツカテゴリ")
        ...
        For Each cat As ContentsCategory In catList
            ...
            sheet.Cell("D" & (count + 9)).Value = "コンテンツカテゴリ設計書通り設定されている" '期待結果
